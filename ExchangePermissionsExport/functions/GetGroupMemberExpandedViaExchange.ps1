Function GetGroupMemberExpandedViaExchange
{
    ...
    $BaseGroupMembership = @(
        foreach ($m in $BaseGroupMemberIdentities) {
            GetTrusteeObject -TrusteeIdentity $m -HRPropertySet $hrPropertySet -ObjectGUIDHash $ObjectGUIDHash -DomainPrincipalHash $DomainPrincipalHash -SIDHistoryHash $SIDHistoryRecipientHash -ExchangeSession $ExchangeSession -UnfoundIdentitiesHash $UnFoundIdentitiesHash
        })
    $iteration = 0
    $AllResolvedMembers = @(
        do
        {
            ...
            $BaseGroupMembership = @(foreach ($m in $BaseGroupMemberIdentities) { GetTrusteeObject -TrusteeIdentity $m -HRPropertySet $hrPropertySet -ObjectGUIDHash $ObjectGUIDHash -DomainPrincipalHash $DomainPrincipalHash -SIDHistoryHash $SIDHistoryRecipientHash -ExchangeSession $ExchangeSession -UnfoundIdentitiesHash $UnFoundIdentitiesHash })
